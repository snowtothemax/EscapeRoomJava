...
/**
 * Main class for the Escape Room game application
 * 
 * @author front
 *
 */
public class EscapeRoom extends PApplet {
  // instance fields
  private PImage backgroundImage;// background image for the application
  private ArrayList<Thing> allThings;// arrayList of all things


  // methods
  /**
   * Main method that runs the applet application
   * 
   * @param args
   */
  ...
  /**
   * setup method with prerequisites for the application window
   */
  public void setup() {
  ...
  /**
   * Method that draws all neccesarry images for the application and removes all inactive objects
   */
  public void draw() {
    image(backgroundImage, 0, 0);
    for (int i = 0; i < allThings.size(); i++)// cycles through the arrayList allThings and runs
                                              // update on each Thing, then if not null, runs act on
                                              // each Action
    {
      Action act = allThings.get(i).update();
      if (act != null) {// acts upon all non null actions
        act.act(allThings);
      }
      if (!allThings.get(i).isActive()) {// removes all inactive objects
        allThings.remove(i);
        i--;
      }
    }
